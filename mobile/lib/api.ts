...
export async function uploadFile(asset: PickedAsset): Promise<UploadResponse> {
  ...
  if (!res.ok) return parseError(res, "Upload failed");
  return res.json();
}
...
export async function processAudio(audioPath: string, prompt: string): Promise<ProcessResponse> {
  ...
  if (!res.ok) return parseError(res, "Processing failed");
  return res.json();
}
...
export async function understandAudio(audioPath: string): Promise<UnderstandResponse> {
  ...
  if (!res.ok) return parseError(res, "Understanding failed");
  return res.json();
}
...
export async function submitJob(type: string, payload: Record<string, unknown>): Promise<{ job_id: string }> {
  ...
  if (!res.ok) return parseError(res, "Failed to submit job");
  return res.json();
}

export async function getJob(jobId: string): Promise<JobResponse> {
  const res = await fetch(`${API_BASE}/api/jobs/${jobId}`);
  if (!res.ok) return parseError(res, "Failed to get job status");
  return res.json();
}
...
export async function exportZip(paths: string[]): Promise<ZipResponse> {
  ...
  if (!res.ok) return parseError(res, "Export failed");
  return res.json();
}
