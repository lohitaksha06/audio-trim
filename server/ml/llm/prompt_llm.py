...
SYSTEM_PROMPT = (
    ...
    "- add_instrument -> params:{instrument} in {drums/bass/guitar/keys/strings/synth/tropical/future/dubstep/edm/other}, {groove} in {default, four_on_floor, funky, swing, half_time, double_time, tropical, future, dubstep, big_room}, {target_bpm}\n"
    "- combine -> params:{instruments} (list of 2+, e.g. [drums, bass] or [tropical, dubstep, edm]) + {groove}\n"
    "- boost -> params:{target} (stem to turn up when user can't hear it)\n"
    "- enhance_vocals -> params:{denoise, clarity} (clearer voices, remove hiss/noise)\n"
    "- style -> params:{style} in {house, tropical, edm, futuristic, dubstep, lofi} + {groove} (DSP preset, not generative)\n"
